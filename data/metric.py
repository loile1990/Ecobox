...
def device_screen_state(folder_path):
    ...
    for filename in files:
        ...
        with open(file_path, "r") as file:
            next(file, None)  # Skip timestamp line

            for line in file:
                line = line.strip()

                # Stop reading at separator
                if line == "----------------------------------------":
                    break

                if "State: On" in line:
                    has_on = True
                elif "State: Locked" in line:
                    has_locked = True
                elif "State: N/A" in line:
                    has_na = True

        # === Decision rules ===
        if has_on:
            results[filename] = 1
        elif has_locked and not has_na:
            # All devices are locked
            results[filename] = 0
        elif has_locked and has_na:
            # Mixed Locked + N/A
            results[filename] = -1
        else:
            # Optional: no valid state found
            results[filename] = -1
